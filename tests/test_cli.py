...
class TestErrorCases:
    ...
    def test_wrong_feature_name_count_exits_one(self, pkl_model_path: pathlib.Path) -> None:
        # Iris has 4 features; supply only 2.
        result = invoke_serve(str(pkl_model_path), "--feature-names", "only_one,only_two")
        assert result.exit_code == 1

    def test_wrong_feature_name_count_error_on_stderr(self, pkl_model_path: pathlib.Path) -> None:
        result = invoke_serve(str(pkl_model_path), "--feature-names", "only_one,only_two")
        stderr = result.stderr or ""
...
class TestHappyPath:
    ...
    def test_exits_zero_with_valid_joblib(self, joblib_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(joblib_model_path))
        assert result.exit_code == 0

    def test_startup_panel_shows_model_name(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path))
        assert "LogisticRegression" in result.output

    def test_startup_panel_shows_default_port(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path))
        assert "8000" in result.output

    def test_startup_panel_shows_swagger_path(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path))
        assert "/docs" in result.output

    def test_startup_panel_shows_predict_path(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path))
        assert "/predict" in result.output

    def test_startup_panel_shows_health_path(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path))
        assert "/health" in result.output

    def test_startup_panel_shows_localhost_not_0000(self, pkl_model_path: pathlib.Path) -> None:
...
class TestArgumentForwarding:
    def test_custom_port_forwarded_to_run(self, pkl_model_path: pathlib.Path) -> None:
        mock = MagicMock()
    ...
    def test_custom_host_forwarded_to_run(self, pkl_model_path: pathlib.Path) -> None:
        mock = MagicMock()
        with patch(_RUN_TARGET, mock):
    ...
    def test_custom_log_level_forwarded_to_run(self, pkl_model_path: pathlib.Path) -> None:
        mock = MagicMock()
        with patch(_RUN_TARGET, mock):
    ...
    def test_custom_port_appears_in_startup_panel(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path), "--port", "9999")
        assert "9999" in result.output

    def test_custom_host_appears_in_startup_panel(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
...
class TestFeatureNamesOption:
    def test_valid_feature_names_accepted(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path), "--feature-names", "a,b,c,d")
        assert result.exit_code == 0

    def test_feature_names_with_spaces_stripped(self, pkl_model_path: pathlib.Path) -> None:
        """Spaces around commas should be tolerated."""
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path), "--feature-names", " a , b , c , d ")
        assert result.exit_code == 0

    def test_named_features_appear_in_panel(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path), "--feature-names", "alpha,beta,gamma,delta")
        assert "alpha" in result.output

    def test_schema_label_is_named(self, pkl_model_path: pathlib.Path) -> None:
        with patch(_RUN_TARGET):
            result = invoke_serve(str(pkl_model_path), "--feature-names", "w,x,y,z")
...
def _assert_kwarg(mock: MagicMock, key: str, expected: Any) -> None:
    ...
    assert actual == expected, f"Expected {key}={expected!r}, got {actual!r}. Full call: {call}"
